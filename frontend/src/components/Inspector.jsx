    ...
    return (
      <div className="inspector-section">
        ...
        {customEntries.length > 0 && (
          <div className="metadata-table" style={{ marginBottom: '12px' }}>
            {customEntries.map(([key, val]) => (
              <div key={key} className="metadata-row" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span className="metadata-key" style={{ flex: '0 0 auto', maxWidth: '90px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{key}</span>
                <span className="metadata-val" style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{formatConfigValue(val)}</span>
                <button
        ...
        <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '6px', lineHeight: 1.4 }}>
          Press Enter or + to add. Custom properties are included in JSON export and simulation payload.
        </p>
      </div>
    );
  };
